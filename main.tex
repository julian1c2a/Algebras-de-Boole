...
\begin{align*}
    a &= a \wedge \top & (ElemNeu_\wedge) \\
      &= a \wedge (a \vee a') & (Comp_\vee) \\
      &= (a \wedge a) \vee (a \wedge a') & (Dist_\wedge) \\
...
\begin{align*}
    a \wedge \bot &= (a \wedge \bot) \vee \bot & (ElemNeu_\vee) \\
               &= (a \wedge \bot) \vee (a \wedge a') & (Comp_\wedge) \\
               &= a \wedge (\bot \vee a') & (Dist_\wedge) \\
               &= a \wedge (a' \vee \bot) & (Comm_\vee) \\
               &= a \wedge a' & (ElemNeu_\vee) \\
               &= \bot & (Comp_\wedge)
\end{align*}
\end{proof}
...
\end{teorema}
\begin{proof}
Supongamos que existe $a \in \mathbb{B}$ tal que $a' = a$. Por el postulado del Complemento ($Comp_\vee$ y $Comp_\wedge$), sabemos que $a \vee a' = \top$ y $a \wedge a' = \bot$. Sustituyendo la hipótesis $a' = a$ en ambas ecuaciones, obtenemos:
...
\begin{teorema}[Asociatividad - $Asoc_\vee, Asoc_\wedge$]
...
\end{align}
\end{teorema}
\begin{proof}[Demostración de $(a \vee b)' = a' \wedge b'$]
Debemos demostrar que $a' \wedge b'$ actúa como el complemento de $(a \vee b)$, satisfaciendo las dos condiciones de complemento. Para la suma:
\begin{align*}
    (a \vee b) \vee (a' \wedge b') &= ((a \vee b) \vee a') \wedge ((a \vee b) \vee b') & (Dist_\vee) \\
    &= ((b \vee a) \vee a') \wedge ((a \vee b) \vee b') & (Comm_\vee) \\
    &= (b \vee (a \vee a')) \wedge (a \vee (b \vee b')) & (Asoc_\vee) \\
...
Para el producto:
\begin{align*}
    (a \vee b) \wedge (a' \wedge b') &= (a' \wedge b') \wedge (a \vee b) & (Comm_\wedge) \\
    &= ((a' \wedge b') \wedge a) \vee ((a' \wedge b') \wedge b) & (Dist_\wedge) \\
    &= (b' \wedge (a' \wedge a)) \vee (a' \wedge (b' \wedge b)) & (Asoc_\wedge, Comm_\wedge) \\
    &= (b' \wedge \bot) \vee (a' \wedge \bot) & (Comp_\wedge) \\
    &= \bot \vee \bot & (Abs_\bot) \\
    &= \bot & (Idemp_\vee)
\end{align*}
Por el teorema de unicidad ($Unic_{comp}$), concluimos que $(a \vee b)' = a' \wedge b'$.
\end{proof}
\begin{proof}[Demostración de $(a \wedge b)' = a' \vee b'$ (Dual)]
Intercambiando operaciones y constantes, demostramos que $a' \vee b'$ es el complemento de $(a \wedge b)$. Para el producto:
\begin{align*}
    (a \wedge b) \wedge (a' \vee b') &= ((a \wedge b) \wedge a') \vee ((a \wedge b) \wedge b') & (Dist_\wedge) \\
    &= ((b \wedge a) \wedge a') \vee ((a \wedge b) \wedge b') & (Comm_\wedge) \\
    &= (b \wedge (a \wedge a')) \vee (a \wedge (b \wedge b')) & (Asoc_\wedge) \\
    &= (b \wedge \bot) \vee (a \wedge \bot) & (Comp_\wedge) \\
    &= \bot \vee \bot & (Abs_\bot) \\
    &= \bot & (Idemp_\vee)
\end{align*}
Para la suma:
\begin{align*}
    (a \wedge b) \vee (a' \vee b') &= (a' \vee b') \vee (a \wedge b) & (Comm_\vee) \\
    &= ((a' \vee b') \vee a) \wedge ((a' \vee b') \vee b) & (Dist_\vee) \\
    &= (b' \vee (a' \vee a)) \wedge (a' \vee (b' \vee b)) & (Asoc_\vee, Comm_\vee) \\
    &= (b' \vee \top) \wedge (a' \vee \top) & (Comp_\vee) \\
    &= \top \wedge \top & (Abs_\top) \\
    &= \top & (Idemp_\wedge)
\end{align*}
Por el teorema de unicidad, concluimos que $(a \wedge b)' = a' \vee b'$.
\end{proof}

\begin{proof}[Demostración de $a \vee b = (a' \wedge b')'$ y su dual]
Partiendo de $(a' \wedge b')'$, aplicamos De Morgan a sus componentes:
\begin{align*}
    (a' \wedge b')' &= (a')' \vee (b')' & (Mor_\wedge) \\
    &= a \vee b & (Comp_{inv})
\end{align*}
Dualizando la expresión, obtenemos de manera idéntica que $(a' \vee b')' = a \wedge b$.
\end{proof}
