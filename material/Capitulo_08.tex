...
\item
  ...
  \item
    ...
    \[n_{D} \in \mathbb{N}\]será \[{n_{D}}^{n_{C}}\]. Así una función de
    \[n\] variables en \[C\]con valores en \[D\]será
    \[{n_{D}}^{({n_{C}}^{n})}\]. Para el caso en que
    \[{C = D} = B_{2}\]que toma como argumento\[n\] variables, obtenemos
    que el número de funciones será de
...
\item
  ...
  \item
    ...
    El primer conjunto \[\{{{({+ , \cdot})},\overline{}}\}\]será el que
    ...
    simples (una variable, o su negada, o una constante). También se
    llamará desarrollo por minitérminos o SOP (inglés) o SdP. El segundo
    será el desarrollo en producto de sumas de términos simples (una
  ...
  \item
    ...
    expresión en que la función tiene como valor \[\mathbf{1}\]. Veamos:
  \item
  ...
  \item
    ...
    \[\mathbf{{n - \mathit{términos}}\mathit{simples}}\]), el valor de
    la función en esa iota concreta indica si el minitérmino aparece o
    no.
  \item
    En el desarrollo por maxitérminos de una función de \[\mathbf{n}\]
    variables, los \[\mathbf{\mathit{maxitérminos}}\] son sumatorios de
    \[\mathbf{{n - \mathit{términos}}\mathit{simples}}\]. Así termina
    consis\-tiendo la función en un producto de maxitérminos. Los
    maxitérminos indican un \[\mathbf{0}\] de la función.
  \item
